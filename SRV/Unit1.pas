...
uses
  ...
  Dialogs, StdCtrls, ScktComp, ExtCtrls, ComCtrls;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure btn_kickallClick(Sender: TObject);
    procedure btn_saveClick(Sender: TObject);
    procedure btn_blockClick(Sender: TObject);
    procedure btn_unblockClick(Sender: TObject);
...
function GetIPbyUsername(user: String) : String;
var
  n: Integer; // loop-counter
begin
  for n := 0 to Clients.Count - 1 do begin
    if TUserData(Clients[n]).Username = User then begin
      result := TUserData(Clients[n]).Host;
      exit;
    end;
  end;
  result := 'UnidentifiedUser';
...
begin
  ...
  201:  //Server SYN
    ...
    Client.Hostname:=Socket.RemoteHost;
...
begin
  //showmessage('lol');
  if accept.Checked then begin
    for n:=0 to block.Count - 1 do begin
      if block.items[n] = Socket.RemoteAddress then begin
        Socket.Close;
        exit;
      end;
    end;
...
var
  ...
  n  : Integer;
begin
  if clients.count = 0 then exit; //Keep it in the bounds!
...
begin
  if GetSelected = #129 then exit;
  showmessage('UID: ' + inttostr(GetIDByUser(GetSelected)) + #13 + #10 + 'Host: ' + TUserData(Clients[GetIDByUser(GetSelected)]).Host + #13 + #10 + 'Hostname: ' + TUserData(Clients[GetIDByUser(GetSelected)]).Hostname  + #13 + #10 + 'Username: ' + TUserData(Clients[GetIDByUser(GetSelected)]).Username);
end;
...
procedure TForm1.btn_kickallClick(Sender: TObject);
var
  n: Integer;
begin
  for n := 0 to clients.count - 1 do
    TCPServer.Socket.Connections[n].Close;
  Clients.Clear;
  userlist.Clear;
end;
...
begin
  if SaveDialog1.Execute then begin
    ...
    rewrite(txtFile);
    writeln(txtFile, output.Text);
    closefile(txtFile);
  end;
end;

procedure TForm1.btn_blockClick(Sender: TObject);
begin
  block.Items.Append(GetIPbyUsername(getSelected()));
end;

procedure TForm1.btn_unblockClick(Sender: TObject);
begin
...
begin
StatusBar1.Panels[0].Text := 'UID: ' + inttostr(GetIDByUser(GetSelected)) + ' - ' + 'Host: ' + TUserData(Clients[GetIDByUser(GetSelected)]).Host + ' - ' + 'Hostname: ' + TUserData(Clients[GetIDByUser(GetSelected)]).Hostname  + ' - ' + 'Username: ' + TUserData(Clients[GetIDByUser(GetSelected)]).Username;
end;
